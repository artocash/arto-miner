		...
		uint32_t startNonce, uint32_t * __restrict__ d_input )
{
	...
	uint32_t tweak1_2[2];
	if (ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_arto || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_bittube2)
	{
	...
	for ( i = start; i < end; ++i )
	{
		...
		for ( int x = 0; x < 2; ++x )
		{
			...
			if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_arto || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_bittube2)
			{
				...
				if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_arto || ALGO == cryptonight_masari || ALGO == cryptonight_bittube2)
				{
            ...
            *( (uint64_t *) t2 ) += sub2 ? ( *( (uint64_t *) t1 ) * *( (uint64_t*) zz ) ) : __umul64hi( *( (uint64_t *) t1 ), *( (uint64_t*) zz ) );
			...
			if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_arto || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_bittube2)
			{
				...
				if (ALGO == cryptonight_arto || ALGO == cryptonight_bittube2)
				{
...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, xmrstak_algo miner_algo, uint32_t startNonce)
{
	...
	static const cuda_hash_fn func_table[] = {
		...
		cryptonight_core_gpu_hash<CRYPTONIGHT_LITE_ITER, CRYPTONIGHT_LITE_MASK, CRYPTONIGHT_LITE_MEMORY/4, cryptonight_arto, 0>,
		cryptonight_core_gpu_hash<CRYPTONIGHT_LITE_ITER, CRYPTONIGHT_LITE_MASK, CRYPTONIGHT_LITE_MEMORY/4, cryptonight_arto, 1>,
