...
There are two easy ways to set variables for `cmake` to configure *arto-miner*
- use the ncurses GUI
  - `ccmake ..`
  - edit your options
...
- `CMAKE_INSTALL_PREFIX` install miner to the home folder
  - `cmake .. -DCMAKE_INSTALL_PREFIX=$HOME/arto-miner`
  - you can find the binary and the `config.txt` file after `make install` in `$HOME/arto-miner/bin`
