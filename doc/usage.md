...
## Usage on Windows
1) Double click the `arto-miner.exe` file
...
2) Start the miner with `./arto-miner`

## Command Line Options

The miner allow to overwrite some of the settings via command line options.
Run `arto-miner --help` to show all available command line options.
...
```
docker run --rm -it -u $(id -u):$(id -g) --name artocash/arto-miner -v "$PWD":/mnt arto-miner
docker stop arto-miner
docker run --rm -it -u $(id -u):$(id -g) --name artocash/arto-miner -v "$PWD":/mnt arto-miner --config config.txt
```

Debug the docker image by getting inside:

```
docker run --entrypoint=/bin/bash --rm -it -u $(id -u):$(id -g) --name artocash/arto-miner -v "$PWD":/mnt arto-miner
```
